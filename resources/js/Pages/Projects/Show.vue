...
const deleteMember = async (membershipId) => {
  ...
  try {
    ...
    memberError.value = err.response?.data?.message || 'メンバーの削除に失敗しました';
  }
};
    ...
    <div class="py-12 px-4">
      <div class="max-w-7xl mx-auto">
        ...
        <nav class="mb-8 flex items-center gap-2 text-sm">
          ...
          <span class="text-slate-800 font-medium">{{ project?.name || '読み込み中...' }}</span>
        </nav>
        ...
        <div class="mb-8 flex items-start justify-between gap-4">
          <div class="flex-1">
            ...
            <div v-if="project" class="flex items-center gap-4 text-sm text-slate-600">
        ...
        <!-- エラー -->
        <div v-else-if="error" class="backdrop-blur-lg bg-red-500/10 border border-red-300/50 rounded-2xl p-6 shadow-xl">
        ...
        <div v-else class="mb-8">
          <div class="backdrop-blur-lg bg-white/60 rounded-2xl shadow-lg border border-white/50 p-2 inline-flex gap-2">
            <button
            ...
            >
              <div class="flex items-center gap-2">
                ...
                タスク
                <span class="px-2 py-0.5 rounded-full text-xs bg-white/20">{{ filteredTasks.length }}</span>
              </div>
            </button>
            <button
            ...
            >
              <div class="flex items-center gap-2">
                ...
                メンバー
                <span class="px-2 py-0.5 rounded-full text-xs bg-white/20">{{ members.length }}</span>
              </div>
            </button>
          </div>
          ...
          <div v-show="activeTab === 'tasks'" class="mt-8 space-y-6">
            <!-- フィルター＆ソートセクション -->
            <div class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-lg border border-white/50 p-6">
              ...
              <!-- フィルター結果 -->
              <div v-if="taskSearchQuery || taskStatusFilter !== 'all'" class="mt-4 flex items-center justify-between">
                <p class="text-sm text-slate-600">
                  <span class="font-semibold text-blue-600">{{ filteredTasks.length }}</span> 件のタスクが見つかりました
                </p>
                <button
            ...
            <!-- タスク作成フォーム -->
            <div class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-xl border border-white/50 p-8">
              <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-2xl font-bold text-slate-800">新しいタスクを作成</h2>
              </div>
              
              <form @submit.prevent="createTask" class="space-y-5">
                <div>
                  <label class="block text-sm font-semibold text-slate-700 mb-2">タスク名</label>
                  <input
                ...
                <div>
                  <label class="block text-sm font-semibold text-slate-700 mb-2">説明（任意）</label>
                  <textarea
                ...
                >
                  <svg v-if="!creatingTask" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                  </svg>
            ...
            <div class="space-y-4">
                ...
                <div class="p-6">
                  <div class="flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                      <h3 class="text-lg font-bold text-slate-800 mb-2 group-hover:text-blue-600 transition-colors">{{ task.title }}</h3>
                      <p v-if="task.description" class="text-slate-600 mb-4 line-clamp-2">{{ task.description }}</p>
                      ...
                      <div class="flex flex-wrap items-center gap-3">
                        ...
                        <span
                          :class="[
                            ...
                            task.status === 'todo' ? 'bg-slate-500/20 text-slate-700 border-slate-300/50' :
                            task.status === 'doing' ? 'bg-amber-500/20 text-amber-700 border-amber-300/50' :
                        ...
                        >
                          <span :class="[
                            ...
                            task.status === 'doing' ? 'bg-amber-500 animate-pulse' :
                            'bg-emerald-500'
                          ]"></span>
                          {{ task.status === 'todo' ? '未着手' : task.status === 'doing' ? '作業中' : '完了' }}
                        </span>
              ...
              <div v-if="filteredTasks.length === 0" class="text-center py-16 backdrop-blur-lg bg-white/60 rounded-2xl border border-white/50">
                <div class="w-20 h-20 mx-auto mb-4 rounded-full backdrop-blur-lg bg-slate-500/10 border border-slate-300/30 flex items-center justify-center">
                  <svg class="w-10 h-10 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                  </svg>
                </div>
                <p class="text-slate-600 font-medium">
                  {{ taskSearchQuery || taskStatusFilter !== 'all' ? '該当するタスクがありません' : 'タスクがまだありません。上のフォームから作成してください。' }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-show="activeTab === 'members'" class="mt-8">
            <div v-if="memberError" class="backdrop-blur-lg bg-red-500/10 border border-red-300/50 rounded-2xl p-6 shadow-xl mb-6">
              <div class="flex items-center gap-3">
                <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
            ...
            <div class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-xl border border-white/50 overflow-hidden">
              <div class="overflow-x-auto">
                <table class="min-w-full">
                  <thead>
                    <tr class="bg-gradient-to-r from-slate-50/80 to-blue-50/80 border-b border-slate-200/50">
                      <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">名前</th>
                      <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">メールアドレス</th>
                      <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">役割</th>
                      <th class="px-6 py-4 text-right text-xs font-bold text-slate-700 uppercase tracking-wider">操作</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-200/50">
                    <tr v-for="membership in members" :key="membership.id" class="hover:bg-blue-50/30 transition-colors">
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-3">
                          ...
                          <span class="text-sm font-semibold text-slate-800">{{ membership.user?.name }}</span>
                        </div>
                      </td>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        <span
                          :class="[
                            ...
                            membership.role === 'project_owner' ? 'bg-purple-500/20 text-purple-700 border-purple-300/50' :
                            membership.role === 'project_admin' ? 'bg-blue-500/20 text-blue-700 border-blue-300/50' :
                            'bg-slate-500/20 text-slate-700 border-slate-300/50'
                          ]"
                        >
                          {{ membership.role === 'project_owner' ? 'オーナー' : 
                             membership.role === 'project_admin' ? '管理者' : 'メンバー' }}
                        </span>
                      </td>
              ...
              <div v-if="members.length === 0" class="text-center py-16">
                ...
                <p class="text-slate-600 font-medium">メンバーが見つかりません</p>
              </div>
            </div>
          </div>
        </div>
