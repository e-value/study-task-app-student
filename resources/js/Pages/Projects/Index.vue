      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <p class="text-slate-600 text-lg">参加中のプロジェクトを管理・閲覧できます</p>
          </div>
        ...
        <!-- 検索＆フィルターセクション -->
        <div class="mb-8 backdrop-blur-lg bg-white/70 rounded-2xl shadow-lg border border-white/50 p-6">
          ...
          <!-- 検索結果数表示 -->
          <div v-if="searchQuery || filterStatus !== 'all'" class="mt-4 flex items-center justify-between">
            <p class="text-sm text-slate-600">
              <span class="font-semibold text-blue-600">{{ filteredProjects.length }}</span> 件のプロジェクトが見つかりました
            </p>
            <button
              @click="clearFilters"
          ...
          >
            ...
            <div class="relative p-6">
              ...
              <div v-if="project.is_archived" class="absolute top-4 right-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium backdrop-blur-lg bg-slate-500/20 text-slate-700 border border-slate-300/30">
              ...
              <!-- プロジェクト名 -->
              <h2 class="text-xl font-bold text-slate-800 mb-4 pr-20 group-hover:text-blue-600 transition-colors duration-300">
                {{ project.name }}
              </h2>

              <!-- メンバー数 -->
              <div class="flex items-center gap-2 text-slate-600 mb-4">
                <div class="p-2 rounded-lg backdrop-blur-lg bg-blue-500/10 border border-blue-300/30">
                  <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                </div>
                <span class="font-medium">{{ project.memberships?.length || 0 }} 人のメンバー</span>
              </div>
              ...
              <div class="flex items-center justify-end">
                <div class="p-2 rounded-full backdrop-blur-lg bg-blue-500/0 group-hover:bg-blue-500/20 border border-blue-300/0 group-hover:border-blue-300/50 transition-all duration-300">
                  <svg class="w-5 h-5 text-blue-600 transform group-hover:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-else class="text-center py-20">
          <div class="backdrop-blur-lg bg-white/60 rounded-3xl p-12 border border-white/50 shadow-xl max-w-md mx-auto">
            <div class="w-24 h-24 mx-auto mb-6 rounded-full backdrop-blur-lg bg-slate-500/10 border border-slate-300/30 flex items-center justify-center">
              <svg class="w-12 h-12 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
              </svg>
            </div>
            <h3 class="text-xl font-bold text-slate-800 mb-2">
              {{ searchQuery || filterStatus !== 'all' ? '該当するプロジェクトがありません' : 'プロジェクトがありません' }}
            </h3>
            <p class="text-slate-600">
              {{ searchQuery || filterStatus !== 'all' ? '検索条件を変更してください' : 'まだどのプロジェクトにも参加していません' }}
            </p>
          </div>
        </div>
      </div>
