...
const confirmDelete = () => {
  if (confirm(`本当に「${task.value.title}」を削除しますか？\n\nこの操作は取り消すことができません。`)) {
    deleteTask();
  }
};
...
const deleteTask = async () => {
  try {
    ...
    // 削除成功後、プロジェクト詳細へ戻る
    router.push({ name: 'project.detail', params: { id: task.value.project.id } });
  } catch (err) {
    ...
    <div class="py-12 px-4">
      <div class="max-w-4xl mx-auto">
        ...
        <template v-else-if="task">
          ...
          <div class="mb-8 flex items-start justify-between gap-4">
            <div class="flex-1">
              <div class="flex items-center gap-3 mb-3">
                <h1 v-if="!editing" class="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
                  {{ task.title }}
                </h1>
                <input
                  v-else
          ...
          <!-- エラー表示 -->
          <div v-if="error" class="mb-6 backdrop-blur-lg bg-red-500/10 border border-red-300/50 rounded-2xl p-6 shadow-xl">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
              ...
              <div class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-xl border border-white/50 p-8">
                ...
                <div v-if="!editing" class="prose prose-slate max-w-none">
                  <p v-if="task.description" class="text-slate-700 whitespace-pre-wrap">{{ task.description }}</p>
            ...
            <div class="space-y-6">
              <!-- ステータス変更 -->
              <div class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-xl border border-white/50 p-6">
                ...
                <div class="space-y-3">
                  ...
                  <p v-if="task.status === 'done'" class="text-center text-sm text-slate-600">
                    このタスクは完了しています
                  </p>
                </div>
              </div>
              ...
              <!-- ステータス編集（編集モード時） -->
              <div v-if="editing" class="backdrop-blur-lg bg-white/70 rounded-2xl shadow-xl border border-white/50 p-6">
                <h3 class="text-lg font-bold text-slate-800 mb-4">ステータス</h3>
                <select
                  v-model="form.status"
              ...
              <!-- 情報 -->
              <div class="backdrop-blur-lg bg-blue-500/10 border border-blue-300/30 rounded-2xl p-6">
                <h3 class="text-sm font-bold text-slate-800 mb-3">タスク情報</h3>
        ...
        <!-- エラー（タスクが見つからない） -->
        <div v-else class="backdrop-blur-lg bg-red-500/10 border border-red-300/50 rounded-2xl p-6 shadow-xl">
          <div class="flex items-center gap-3">
            <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
