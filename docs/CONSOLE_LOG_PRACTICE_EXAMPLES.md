...
### 🎓 基礎編：handleError を使わないバージョン

**まずは基礎から！handleError を使わずに、生の console.log でエラーを確認する方法を学びましょう。**
...
const fetchTask = async () => {
    ...
    try {
        ...
        console.log("💬 Laravel のメッセージ:", response.data.message);
        ...
        if (err.response) {
            ...
            console.error("📦 完全なレスポンスデータ:", err.response.data);
            ...
            switch (err.response.status) {
                ...
                case 403:
                    ...
                    console.error(
                        "💡 確認事項: このタスクへのアクセス権限がありますか？"
                    );
                    break;
                case 500:
                    console.error("💥 500エラー: サーバー内部エラー");
                    console.error(
                        "💡 確認事項: storage/logs/laravel.log を確認してください"
                    );
                    break;
            }
        } else if (err.request) {
            ...
            console.error("💡 確認事項:");
            console.error(
                "  - Laravel サーバーは起動していますか？（php artisan serve）"
            );
            console.error("  - Vite は起動していますか？（npm run dev）");
...
### 🚀 実践編：handleError と組み合わせたバージョン

**handleError を使うと、エラーハンドリングが統一されて保守性が向上します。**

**現在のコード（30-42 行目）を以下に置き換えてください：**
...
const fetchTask = async () => {
    ...
    try {
        ...
        if (err.response) {
            ...
            console.error("💡 確認事項:");
            console.error(
                "  - サーバーは起動していますか？（php artisan serve）"
            );
            console.error("  - ネットワーク接続は正常ですか？");
        } else {
            // リクエストの設定中にエラーが発生
...
**現在のコード（105-124 行目）を以下に置き換えてください：**
        ...
        if (err.response) {
            ...
            switch (err.response.status) {
                ...
                case 500:
                    ...
                    console.error(
                        "  - storage/logs/laravel.log を見てください"
                    );
                    break;
            ...
            console.error("  ✓ サーバーは起動していますか？");
            console.error(
                "    → ターミナルで 'php artisan serve' を実行してください"
            );
            console.error("  ✓ ネットワーク接続は正常ですか？");
...
1. タスク ID を存在しない ID に変更（例：99999）
2. ブラウザをリロード
3. Console でエラーログを確認
4. Network タブで 404 エラーを確認
