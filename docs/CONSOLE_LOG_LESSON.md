...
**ガネーシャ 🐘**：「実はな、よくあるミスで**変数名のタイポ（スペルミス）**ってのがあるんや。`taskId`を`taskid`（小文字のi）って書いてまうとか」
...
**ガネーシャ 🐘**：「せや！これが**JavaScriptの変数未定義エラー（ReferenceError）**や。`taskid`って変数は存在せえへんから、エラーになっとるんや」

**生徒 👩‍💻**：「あ！`taskId`（大文字のI）のはずなのに、`taskid`（小文字のi）って書いてしまったからですね！」
