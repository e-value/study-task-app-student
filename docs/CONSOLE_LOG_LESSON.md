...
## 📢 このプロジェクトの通知機能（Toast）について

**ガネーシャ 🐘**：「まず最初に大事なことを教えるで。このプロジェクトでは `alert()` の代わりに `toast` っていう通知機能を使うんや」

**生徒 👩‍💻**：「toast...ですか？」

**ガネーシャ 🐘**：「せや！`toast` は画面右上におしゃれな通知を出してくれる機能や。`alert()` みたいにダイアログで画面を止めへんから、ユーザー体験が良いんや」
...
**ガネーシャ 🐘**：「このプロジェクトのコードでは、すでに `toast` が使えるように設定されとるから、そのまま使ってええで！」

**生徒 👩‍💻**：「わかりました！教材のコード例をそのままコピペすれば動くんですね」

**ガネーシャ 🐘**：「せやせや！教材を読みながら、コピペして試してみてな」
...
**生徒 👩‍💻**：「そういえば、このプロジェクトには `toast` っていう通知機能がありますよね？」

**ガネーシャ 🐘**：「おお、よう気づいたな！`toast` は画面右上に通知を出す便利な機能や。このプロジェクトでは `alert` の代わりに `toast` を使うのがルールやで」
