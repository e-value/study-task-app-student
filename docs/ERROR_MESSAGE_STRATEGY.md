...
**定義**: システムのインフラ層で発生するエラー（DB 接続失敗、メモリ不足など）
...
**定義**: インターネット接続切断、DNS エラー、タイムアウトなど
...
→ ユーザーには必要最小限の情報のみ。管理者は Sentry で詳細確認
...
-   `app/Http/Responses/ApiResponse.php` - API レスポンス共通化
...
| エラー種類                 | 管理           | 本番メッセージ       | 開発メッセージ       | Sentry ログ |
| -------------------------- | -------------- | -------------------- | -------------------- | ----------- |
| 技術的エラー（500）        | バックエンド   | "サーバーエラー"     | 詳細                 | ✅          |
| ビジネスロジック（400 系） | バックエンド   | メッセージ           | メッセージ           | -           |
